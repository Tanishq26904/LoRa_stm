...
uint8_t parse_REQ_DATA(const char *rx, uint8_t *dst, uint8_t *src)
{
    // Find command first (fast reject)
    if (strstr(rx, "REQ_DATA") == NULL)
        return 0;

    // Parse destination ID (first number)
    if (!(rx[0] >= '0' && rx[0] <= '9' &&
          rx[1] >= '0' && rx[1] <= '9'))
        return 0;
